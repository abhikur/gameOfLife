...
Grid.prototype.populateParticular = function(position) {
    var self = this;
    self.grid[position.x][position.y] = 1;
}
...
Grid.prototype.liveCells = function() {
    var indices = [];
    for(i=0; i < this.size.h; i++) {
        for (var j=0; j < this.size.w; j++) {
            if(this.grid[i][j] == 1) indices.push('' + i + j);
        }
    }
    return indices;
}

Grid.prototype.deadCells = function() {
    var indices = [];
    for(i=0; i < this.size.h; i++) {
        for (var j=0; j < this.size.w; j++) {
            if(this.grid[i][j] == 0) indices.push('' + i + j);
        }
    }
    return indices;
}
